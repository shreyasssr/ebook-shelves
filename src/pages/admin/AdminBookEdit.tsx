...
export default function AdminBookEdit() {
  ...
  useEffect(() => {
    supabase.from("languages").select("*").order("display_order").then(({data})=>setLangs(data ?? []));
    supabase.from("categories").select("*").order("name").then(({data})=>setCats(data ?? []));
    if (!isNew && id) {
      supabase.from("books").select("*").eq("id", id).maybeSingle().then(({data})=>{ if (data) setBook(data); });
    }
  }, [id, isNew]);
  ...
  const save = async () => {
    if (!book.name || !book.author || !book.description || !book.language_id || !book.price) {
      toast.error("Fill required fields"); return;
    }
    setBusy(true);
    const payload: any = { ...book, slug: book.slug || slugify(book.name) };
    delete payload.search_vector;
    const res = isNew
      ? await supabase.from("books").insert(payload).select().single()
      : await supabase.from("books").update(payload).eq("id", id!).select().single();
    setBusy(false);
    if (res.error) toast.error(res.error.message);
    else { toast.success("Saved"); nav(`/admin/books/${res.data.id}`, { replace: true }); }
  };
